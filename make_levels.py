...
pygame.init()
levels=[{'rows':20,'cols':40},{'rows':60,'cols':60},{'rows':60,'cols':20},{'rows':20,'cols':60},{'rows':40,'cols':40},{'rows':47,'cols':44}]
level = 6
main_page  = 0 
...
boss_img=pygame.transform.scale(pygame.image.load('bacteria.jpeg'),(2*tile_size,2*tile_size))
        ...
        for col in range(levels[level-1]['cols']):
            if world_data[row][col] > 0:
                if world_data[row][col] == 1:
                    ...
                    intermediate.blit(boss_img, (col * tile_size, row * tile_size) )
